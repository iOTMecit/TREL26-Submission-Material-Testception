...
For a fresh Ubuntu 24.04 installation, install the base development dependencies first:
...
Check the available Java installations:

```bash
ls -la /usr/lib/jvm/
```

Typical Java paths on Ubuntu `amd64` are:
...
Typical Java paths on Ubuntu `arm64` are:
...
Verify both Java versions using the paths available on your machine.
...
Testception uses the environments as follows:
...
Install Docker Engine using the official Docker instructions for Ubuntu, then verify:
...
Ubuntu `arm64` environments, including Ubuntu virtualized with UTM on Apple Silicon Macs, can be used for development.
...
The project has primarily been evaluated on Ubuntu `amd64`, so on ARM64 you should also verify that the required browser, Docker images, Java paths, and subject-specific binaries are available for the architecture.
...
`STEP_SKIPPED` messages are suppressed from the live terminal view when progress mode is active, but remain available in the log. The final progress summary reports the total skip count.
...
Large raw coverage traces should remain local unless explicitly selected as research artifacts.

...
PROFILE_MODE="${TESTCEPTION_COVERAGE_PROFILE_MODE:-sourcemap}"
...
# TESTCEPTION_DIMESHIFT_LEGACY_POSTPROCESS_V1
if [[ "${TESTCEPTION_RUN_POSTPROCESS:-true}" == "true" ]]; then
    echo
    echo "Running coverage post-processing..."

    if [[ "$PROFILE_MODE" == "legacy-multiscript" ]]; then
        "$PYTHON_BIN" "$SCRIPT_DIR/dimeshift_legacy_postprocessor.py" \

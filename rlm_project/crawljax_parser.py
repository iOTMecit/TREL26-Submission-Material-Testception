...
def normalize_xpath(xpath):
    """Normalize XPath values before comparing parser and result.json locators."""
    if not xpath:
        return ""
...
def generate_worker_prompt(
    ...
    elements,
    visited_ids,
    preferred_xpaths=None,
):
    """
    Convert the actionable DOM skeleton into an LLM-readable prompt.

    Hybrid exploration markers:
    - RECORDED_UNTRIED:
        result.json edge exists and has not been explored yet.
    - RECORDED_EXPLORED:
        result.json edge exists but was already explored.
    - DOM_DISCOVERY_CANDIDATE:
        action exists in the DOM but is not an untried result.json edge.
    """
    ...
    for el in elements:
        status = (
            "[ALREADY_TRIED] "
            if el["llm_id"] in visited_ids
            else ""
        )

        normalized_xpath = normalize_xpath(
            el.get("xpath", "")
        )

        normalized_absolute_xpath = normalize_xpath(
            el.get("absolute_xpath", "")
        )

        # Backward compatibility:
        # preferred_xpaths contains currently untried result.json edges.
        is_recorded_untried = (
            el.get(
                "crawl_transition_candidate",
                False,
            )
            or normalized_xpath in preferred_xpaths
            or normalized_absolute_xpath
            in preferred_xpaths
        )

        is_recorded_explored = el.get(
            "recorded_transition_explored",
            False,
        )

        is_dom_discovery = el.get(
            "dom_discovery_candidate",
            False,
        )

        if is_recorded_untried:
            priority_marker = (
                "[RECORDED_UNTRIED] "
            )

        elif is_recorded_explored:
            priority_marker = (
                "[RECORDED_EXPLORED] "
            )

        elif is_dom_discovery:
            priority_marker = (
                "[DOM_DISCOVERY_CANDIDATE] "
            )

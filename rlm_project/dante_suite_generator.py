...
def generate_dante_suites(
    ...
    java_start_url = (
        f'"{escape_java_string(start_url)}"'
        if start_url
        else "Properties.app_url"
    )
    output_path = (
        Path(output_dir).resolve()
        if output_dir
...
"""
    ...
    for i, scenario in enumerate(valid_scenarios):
        ...
        normal_code += f"\t\tdriver.get({java_start_url});\n"
        normal_code += f"\t\tThread.sleep({wait_time});\n"
        scenario_blob = " ".join(
            [
                (st.get("element", "") or "").lower()
                + " "
                + (st.get("feature_signature", "") or "").lower()
                + " "
                + (st.get("xpath", "") or "").lower()
                for st in scenario
            ]
        )

        skip_login_for_public_flow = any(k in scenario_blob for k in [
            "create new account",
            "sign up",
            "signup",
            "registration"
        ])
        if not skip_login_for_public_flow:
        ...
        coverage_code += f"\t\tdriver.get({java_start_url});\n"
...
"""
    ...
    normal_file = (
        output_path
        / f"{class_name_normal}.java"
    )

    coverage_file = (
        output_path
        / f"{class_name_coverage}.java"
    )
    ...
    print(
        "✅ Normal test suite oluşturuldu: "
        f"{normal_file}"
    )

    print(
        "✅ Coverage test suite oluşturuldu: "
        f"{coverage_file}"
    )

    print(
        "✅ SUCCESS! LLM-centric Dante-compatible "
        f"Java tests generated. "
